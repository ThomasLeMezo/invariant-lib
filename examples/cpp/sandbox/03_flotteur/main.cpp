...
double friction = 0.1;
double input_coeff = 0.001;

Vector2d f(const Vector2d &x, const double &u, const bool &compressibility=false, const double &chi=1e-4)
{
    Vector2d dx;
    dx(0) = x(1); // Depth
    dx(1) = -input_coeff*u-friction*abs(x(1))*x(1)+(compressibility?(chi*(x(0)-depth_equilibrium)):0.0); // Velocity
    return dx;
}
...
void example1()
{
    ...
    const double frame_rate = 15; // frame/sec
    ...
    x2.push_back(x(0));
    u_log.push_back(0.0);
    for(double t=t_init; t<global_t_max; t+=dt)
    {
        // Command
        if(t<30.0)
            u=0;
...
//        cout << t << " " << x(0) << " " << x(1) << endl;
    ...
    for(double max_t = t_init; max_t <global_t_max; max_t+=dt_export)
    {
        ...
        ipegenerator::Figure fig(frame, 112, 63);
        fig.set_inverted_y();
        ...
        size_t offset = max((size_t)1,(size_t)round((max_t-t_init)/dt))+1;
        vector<double> x1_sub = vector<double>(x1.begin(), x1.begin()+offset);
        vector<double> x2_sub = vector<double>(x2.begin(), x2.begin()+offset);
        vector<double> u_sub = vector<double>(u_log.begin(), u_log.begin()+offset);
        ...
        fig.draw_float(x1_sub.back(), x2_sub.back(),u_sub.back(), 0.0, ipegenerator::FLOAT_PISTON_EQUAL, 0.25);

        fig.draw_axis("t", "z");
        std::stringstream time_text;
        time_text << "$u=" << u_sub.back() << "$";
...
void example5_1()
{
    ...
    for(double max_t = t_init; max_t<global_t_max; max_t +=dt_export)
    {
        ...
        // Mid Box
        depth = 0.5+(max_t/global_t_max)*2.;
        if(depth>=1.5)
        {
            double depth_local = 1.5;
            compressibility = width_max*(1-compressibility_factor_water*(depth_local/max_depth));
            fig.draw_box(ipe::Vector(x, depth_local), compressibility, true);
        }
...
void example5_3()
{
    const string directory = "/home/lemezoth/Videos/thesis/animation5/imgs3/";
    const double t_init = 0.0;
