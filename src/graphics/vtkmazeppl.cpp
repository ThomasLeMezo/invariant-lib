...
#pragma omp for schedule(dynamic)
        for(int pave_id=0; pave_id<dim_paves_list; pave_id++){
            ...
            if((!complementary && !r->is_empty()) || (complementary && !r->is_full())){
                nb_pave_not_empty++;
                vtkSmartPointer<vtkPoints> points = vtkSmartPointer<vtkPoints>::New();
...
#define VTK_MAZE_PPL_POLYGON_REPRESENTATION 1
#if VTK_MAZE_PPL_POLYGON_REPRESENTATION
...
void VtkMazePPL::simu_trajectory(invariant::Dynamics &d, const ibex::IntervalVector &space, const size_t &nb_traj, const double &t_max, const double &dt, const vector<double> &visu_scale){

    cout << "Simulation of trajectories" << endl;
    double time_start = omp_get_wtime();
    vector<array<double, 3>> init_conditions;

    for(double x_traj=space[0].lb(); x_traj<=space[0].ub(); x_traj+=space[0].diam()/(nb_traj-1)){
        for(double y_traj=space[1].lb(); y_traj<=space[1].ub(); y_traj+=space[1].diam()/(nb_traj-1)){
            for(double z_traj=space[2].lb(); z_traj<=space[2].ub(); z_traj+=space[2].diam()/(nb_traj-1)){
                init_conditions.push_back(array<double, 3>{x_traj, y_traj, z_traj});
            }
        }
    }
...
#pragma omp critical
        {
            cout << "=> compute trajectory " << k << endl;
        }

        vtkSmartPointer<vtkPolyData> linesPolyData = vtkSmartPointer<vtkPolyData>::New();
        vtkSmartPointer<vtkPoints> points = vtkSmartPointer<vtkPoints>::New();
        ibex::IntervalVector x(3);
        x[0] = ibex::Interval(init_conditions[k][0]);
        x[1] = ibex::Interval(init_conditions[k][1]);
        x[2] = ibex::Interval(init_conditions[k][2]);

        for(double t=0; t<t_max; t+=dt){
            ibex::IntervalVector dx = d.eval(x)[0];
